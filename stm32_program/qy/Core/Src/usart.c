...
void HAL_UART_Printf(UART_HandleTypeDef *huart, char *fmt, ...)
{
    ...
    va_start(ap, fmt);
    vsnprintf(UsartPrintfBuf, sizeof(UsartPrintfBuf), fmt, ap);    // 格式化字符串
    va_end(ap);

    HAL_UART_Transmit(huart, (uint8_t *)UsartPrintfBuf, strlen(UsartPrintfBuf), HAL_MAX_DELAY);  // 使用HAL库函数发送数据
}
/* USER CODE END 1 */
