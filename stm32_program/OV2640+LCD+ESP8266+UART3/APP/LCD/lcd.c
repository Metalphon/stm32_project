...
{   
	...
	if(lcddev.dir==0&&lcddev.id==0X1963)  //1963竖屏处理
		{
			X0=lcddev.width-width-X0; 
			height=Y0+height-1; 
			LCD_CMD=lcddev.setxcmd;      //设置x的坐标
			LCD_DATA=X0>>8; 
			LCD_DATA=X0&0XFF;	 
			LCD_DATA=(X0+width-1)>>8; 
			LCD_DATA=(X0+width-1)&0XFF;  
			LCD_CMD=lcddev.setycmd;      //设置y的坐标
			LCD_DATA=Y0>>8; 
			LCD_DATA=Y0&0XFF; 
			LCD_DATA=height>>8; 
...
void Set_Scan_Direction(u8 direction)
{
	...
	if( (lcddev.dir==1&&lcddev.id==0X9341)||(lcddev.dir==0&&lcddev.id==0X1963)) 
	  ...
	  LCD_Open_Window(0,0,lcddev.width,lcddev.height);  //设置完扫描方向后，开显示区域为全屏窗口
}
...
{ 	
   ...
   lcddev.id=ILI9341_Read_id(); //先读看看所接屏幕是不是9341驱动
		
	 if(lcddev.id!=0x9341)		//如果不是9341，读看看是不是1963驱动	 
		{
			...
			if(lcddev.id==0x5761)
				 lcddev.id=0x1963;  //SSD1963实际读出的ID是0x5761,为了直观，这边设置为1963
		}
...
{  							  
  ...
  //cfont.h中字库是从空格开始的 空格就是第一个元素 其他字符的ASCII码减去空格后就得到在数组中的偏移值(位置) 
	    ...
	    {   
					...
					for(a=0;a<8;a++)   
					{	   
						if(bytedata&0x01) LCD_Color_DrawPoint(x,y,POINT_COLOR); //由于子模是低位在前 所以先从低位判断  为1时显示画笔颜色      
